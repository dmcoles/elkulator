...
//--------------------------------------
			...
			if (x < width - 3) {
				x += 3;
				if (PixelsPerMask == 2) {
					color[3] = *(((uint16_t*)src_line[0]) + x);
					color[7] = *(((uint16_t*)src_line[1]) + x);
					color[11] = *(((uint16_t*)src_line[2]) + x);
					color[15] = *(((uint16_t*)src_line[3]) + x);
				}
				else {
...
//                                            A1 A2          10 11
			...
			if (x < width - 2) {
				x += 2;
				if (PixelsPerMask == 2) {
					color[1] = *(((uint16_t*)src_line[0]) + x);
					if (x < width) {
						color[5] = *(((uint16_t*)src_line[1]) + x + 1);
						color[9] = *(((uint16_t*)src_line[2]) + x + 1);
					}
					color[11] = *(((uint16_t*)src_line[3]) + x);
				}
				else {
